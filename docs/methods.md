...
Retained sequences were aligned to the Wuhan-Hu-1 reference genome (NC_045512.2) using Nextclade v3 (https://clades.nextstrain.org), which performs independent pairwise alignment of each sequence. The Nextclade dataset for SARS-CoV-2 was used to provide the reference sequence, gene map, and quality control parameters. Sequences receiving a Nextclade overall QC status of "bad" were excluded, retaining 5,635,025 rows with "good" or "mediocre" QC status. We then removed 1,883 sequences identified on manual audit as wastewater samples, cell-passaged (Vero) isolates, or sequences with collection dates preceding their Pango lineage's earliest documented emergence (see "Audit of problematic sequences" below). Finally, to ensure each sequence contributed exactly once to downstream counts, we de-duplicated the Nextclade output by GISAID accession ID, collapsing 4,273 redundant rows arising from sequences that appeared in multiple Nextclade output lines. The final analysis cohort comprised **5,628,868 unique sequences**.

From the Nextclade output, we extracted nucleotide substitutions, amino acid substitutions, and insertions/deletions. For each mutation, we computed the pandemic-wide frequency as the number of sequences carrying that mutation divided by the total number of sequences (5,628,868), expressed as a percentage. Mutations were retained for analysis if their pandemic-wide frequency exceeded 0.01% (approximately 563 or more sequences). This threshold yielded 10,147 unique nucleotide substitutions and 5,665 unique amino acid substitutions.
...
To further ensure data integrity, we cross-referenced our filtered dataset against the 42,419 GISAID sequences flagged as "under investigation" (accessed March 22, 2026; human host, collection date December 1, 2019 to January 1, 2026). Of these, 41,690 were present in our raw metadata download (17,564,626 sequences). None passed our quality control filters: zero flagged sequences were present in our filtered dataset (5,732,294 sequences) or final analysis cohort (5,628,868 sequences). All flagged sequences were excluded at the prefilter stage, primarily by the high-coverage requirement.
